...
(function(factory){
  ...
  var DPGlobal = {
    ...
    parseDate: function (date, format, language, type) {
      ...
      var parts = [];
      if(date.match(/\d{8}/g)){//yyyymmdd, 正则表达式不会写，略挫的方法搞出来先
         parts[0] = date.toString().substring(0,4);
         parts[1] = date.toString().substring(4,6);
         parts[2] = date.toString().substring(6,8);
      }else if(date.match(/\d{6}/g)){//yyyymm
         parts = date && date.toString().match(/(\d{4})|(\d{2})/g) || [];
      }else {
